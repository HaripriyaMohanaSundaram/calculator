  ...
  switch (true) {
    case (!isNaN(numberValue)):
      ...
      if(numberArray.length==1){
        displayText =String( numberArray[numberArray.length - 1]);
      }
      else{
        displayText = String(numberArray[numberArray.length - 2]) +" "+operatorArray[operatorArray.length - 1] +" " + numberArray[numberArray.length - 1];
      }
      // document.getElementById("displayScreen").innerText=displayText;
      document.getElementById("deleteBtn").disabled = false;
      break;
...
function getInput(numberValue, number, isDecimal, exponent) {
  if (number == 0) {
    if (isDecimal == false) {
      ...
      numberValue = numberValue * (0.1 ** exponent).toFixed(exponent);
...
function deleteValue(numberArray, operatorArray, inputArray, exponent) {
  ...
  switch (true) {
    case (!isNaN(inputArray[inputArray.length - 1])):
      if (updateNumber != 0) {
        if (isDecimal == true) {
          inputArray[inputArray.length - 1] = inputArray[inputArray.length - 1] * (0.1 ** exponent).toFixed(exponent);
